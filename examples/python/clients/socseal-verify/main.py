...
The client answers the 402 challenge, pays, retries, and receives a
...
async def paid_get(http, path: str) -> dict:
    """GET with automatic 402 -> exact-USDC-pay -> retry via the client."""
...
async def main() -> None:
    ...
    if not key:
        print("EVM_PRIVATE_KEY required")
        sys.exit(1)
